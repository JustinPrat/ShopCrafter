...
namespace ScriptMaker.Editor
{
    public class ScriptMaker : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("MakeScript", GUILayout.Height(20)) && template != null)
            {
                GenerateScript(scriptName, path, template);
            }
        }

        private void GenerateScript(string name, string path, ScriptMakerTemplate template, bool refresh = true)
        {
            ...
            string fullPath = Path.Combine(path, className + template.ScriptNameSuffix + ".cs");
            ...
            File.WriteAllText(fullPath, template.GetScriptTemplate(name));
